...
int parse_inputs(PrayerTimes *prayerTimes, int argc, char **argv) {
    for (int i = 1; i < argc; i++) {
        if (!(strcmp(argv[i], "--help") && strcmp(argv[i], "-h"))) {
            ...
            ++i;
            VALUE_REQUIRED(i, argc, argv[i - 1]);
            if (sscanf(argv[i], "%d:%d", &new_date->tm_hour,
                       &new_date->tm_min) != 2) {
            ...
            ++i;
            VALUE_REQUIRED(i, argc, argv[i - 1]);
            if (sscanf(argv[i], "%d-%d-%d", &new_date->tm_mday,
                       &new_date->tm_mon, &new_date->tm_year) != 3) {
            ...
            if (handle_double_option(argc, &i, argv, &prayerTimes->timezone,
                                     argv[i]))
                return -1;
        } else if (!(strcmp(argv[i], "--latitude") && strcmp(argv[i], "-u"))) {
            if (handle_double_option(argc, &i, argv, &prayerTimes->latitude,
                                     argv[i]))
                return -1;
            ...
            if (handle_double_option(argc, &i, argv, &prayerTimes->longitude,
                                     argv[i]))
                return -1;
        } else if (!(strcmp(argv[i], "--calc-method") &&
                     strcmp(argv[i], "-c"))) {
            ++i;
            VALUE_REQUIRED(i, argc, argv[i - 1]);
            ...
            ++i;
            VALUE_REQUIRED(i, argc, argv[i - 1]);
            if (handle_array_options(argv[i - 1], Juristic,
                                     JURISTICMETHOD_COUNT, argv[i],
                                     &prayerTimes->asr_juristic))
                return -1;
        } else if (!(strcmp(argv[i], "--high-lats-method") &&
                     strcmp(argv[i], "-i"))) {
            ++i;
            VALUE_REQUIRED(i, argc, argv[i - 1]);
            if (handle_array_options(argv[i - 1], Adjusting,
                                     ADJUSTINGMETHOD_COUNT, argv[i],
                                     &prayerTimes->adjust_high_lats))
                return -1;
        } else if (!strcmp(argv[i], "--dhuhr-minutes")) {
            if (handle_double_option(argc, &i, argv,
                                     &prayerTimes->dhuhr_minutes, argv[i - 1]))
                return -1;
        } else {
...
int print_help(const char *arg) {
    if (!arg) {
        puts("Usage: " PROG_NAME
             ...
             "    -v, --version                   Show the name and version of "
             "the "
             "program\n"
        ...
        printf(
            ...
            "    -p, --previous                  Print the previous prayer "
            "time.\n"
        ...
        printf(
            "Date Options:\n"
            "    -d, --date <arg>      Get prayer times for a specific date "
            "arg "
            "format: "
            "DD-MM-YYYY\n"
            "    -t, --time <arg>      Get prayer times for a specific time "
            "arg "
            "format: "
            "HH:MM\n"
        ...
        printf(
            ...
            "    -g, --longitude <arg>      Longitude of the desired "
            "location.\n"
            "\n");
        return 1;
        ...
        printf(
            ...
            "    -c, --calc-method <arg>          Select prayer time "
            "calculation "
            "method.\n\n"
            "Calculation Method Options:\n"
            "    Jafari            Ithna Ashari\n"
        ...
        printf(
            ...
            "    -j, --asr-juristic <arg>          Select Juristic method for "
            "Asr "
            "prayer time.\n\n"
            "Asr Juristic Method Options:\n"
        ...
        printf(
            ...
            "    -i, --high-lats-method <arg>          Select adjustment "
            "method "
            "for "
            "higher latitudes.\n"
            "\n"
...
void print_next_prayer(PrayerTimes *prayerTimes) {
    ...
    for (TimeID timeid = 0; timeid < TIMEID_TimesCount; timeid++) {
        ...
        if (mktime(&times_dates[timeid]) > prayerTimes->time) {
            Time diff = convert_time_hms(mktime(&times_dates[timeid]) -
                                         prayerTimes->time);
            printf(
    ...
    Time diff =
        convert_time_hms(mktime(&times_dates[TIMEID_Fajr]) - time(NULL));
    printf(
        "Upcoming Notification:\nNext Prayer: %s: %2.2d:%2.2d\nTime "
        "Remaining: %2.2d:%2.2d:%2.2d\n",
...
void print_previous_prayer(PrayerTimes *prayerTimes) {
    ...
    for (TimeID timeid = TIMEID_TimesCount - 1; timeid > 0; timeid--) {
        ...
        get_float_time_parts(times[timeid], &times_dates[timeid].tm_hour,
                             &times_dates[timeid].tm_min);
        if (mktime(&times_dates[timeid]) < prayerTimes->time) {
            Time diff = convert_time_hms(prayerTimes->time -
                                         mktime(&times_dates[timeid]));
            printf(
    ...
    Time diff =
        convert_time_hms(time(NULL) - mktime(&times_dates[TIMEID_Isha]));
    printf(
        "Previous Prayer: %s: %2.2d:%2.2d\nTime "
        "Elapsed: %2.2d:%2.2d:%2.2d\n",
